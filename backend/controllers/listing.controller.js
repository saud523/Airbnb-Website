...
export const addListing = async (req, res) => {
  try {
    ...
    const listing = await Listing.create({
      ...
      rent,
      city,
      landmark,    // ⭐ must match schema name
      category,
      image1: `${process.env.SERVER_URL}/uploads/${req.files["image1"][0].filename}`,
image2: `${process.env.SERVER_URL}/uploads/${req.files["image2"][0].filename}`,
image3: `${process.env.SERVER_URL}/uploads/${req.files["image3"][0].filename}`,

      host: req.user._id   // ⭐ important for required field
    });
